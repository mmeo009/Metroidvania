...
public class CharacterMovement : MonoBehaviour
{
    ...
    private void FixedUpdate()                      // Fixedupdate에서 이동 구현을 한다
    {
        ...
        // GroundeCheck 위치에서 땅으로 지정된 것과 겹치는지 확인
        ...
        if (!m_IsGrounded) OnFallEvent.Invoke();                // 땅이 아니라 떨어지고 있다는 이벤트를 호출
        ...
        for(int i = 0; i < wallColliders.Length; i++)
        {
            if(wallColliders[i].gameObject != gameObject)
            {
                ...
                if(!m_IsGrounded && !status.isDashing && m_Rigidbody2D.velocity.y < 0)
                {
                    ...
                    if(!wasWallSlidding)            // 이전 프레임이 벽이 아니였다면 체그해서 돌려서 벽을 슬라이딩 하고 검출하게 한다.
                    {
        ...
        if(!m_IsWallSlidding && wasWallSlidding)
        {
            ...
            animator.SetBool("IsWallSlidding", false);
        }

        animator.SetFloat("Speed", Mathf.Abs(m_Rigidbody2D.velocity.x));        // 에니메이터에 속도 전달
    }
    public void Move(float move, bool jump, bool dash)
    {
        if(status.canMove)                                  // 클래스에서 이동이 가능하다고 하면
        {
            if(dash && status.canDash && !m_IsWallSlidding) // 대쉬가 가능한 조건을 검사하여 대쉬 실행
            {
                status.StartDash(0.1f);
            }

            if(status.isDashing)                            // 대쉬 중일 때 처리
            {
                m_Rigidbody2D.velocity = new Vector2(transform.localScale.x * status.mDashForce, 0);
            }
            else if(m_IsGrounded || m_AirControl)           // 플레이어가 땅에 닿았거나 공중 제어가 켜져있을 때
            {
                if (m_Rigidbody2D.velocity.y < -limitFallSpeed) // 떨어지는 속도가 너무 빠르면 속도 제한
                    m_Rigidbody2D.velocity = new Vector2(m_Rigidbody2D.velocity.x, -limitFallSpeed);

                Vector2 targetVelocity = new Vector2(move * 10.0f, m_Rigidbody2D.velocity.y);       // 목표 속도를 계산
                ...
                if(move > 0 && !m_FacingRight && !m_IsWallSlidding)     // 입력이 오른쪽, 플레이어가 왼쪽을 보고 있다면
                {
                    Flip();                                             // 플레이어를 뒤집는다.
                }
            ...
            else if(m_IsWall && !m_IsGrounded)
            {
                if(!oldWallSlidding && m_Rigidbody2D.velocity.y < 0 || status.isDashing)        // 벽에 붙어서 슬라이딩 되는 순간
                {
                    ...
                    canDoubleJump = true;
                    animator.SetBool("IsWallSlidding", true);
                }
                ...
                if(jump && m_IsWallSlidding)            // 벽 슬라이딩 중 점프
                {
                    ...
                    m_Rigidbody2D.AddForce(new Vector2(transform.localScale.x * m_JumpForce * 1.2f, m_JumpForce));      // 점프 힘을 전달 해준다.
